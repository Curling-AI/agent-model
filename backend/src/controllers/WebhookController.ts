import { getByFilter, getById, update, upsert } from "@/services/storage";
...
export const WebhookController = {
          ...
          for (const change of entry.changes) {
            ...
            if (messages && Array.isArray(messages)) {
              for (const message of messages) {
                ...
                const integrations = await getByFilter("integrations", {
                ...
                if (conversation["mode"] === "agent") {
                  ...
                  if (type === "video" || type === "document") {
                    ...
                    responseMessage = await AiExecutor.executeAgentMedia(
                      agent["id"],
                    ...
                    responseMessage = await AiExecutor.executeAgentText(
                      agent["id"],
                      conversation["lead_id"],
                      text,
                    );
                  ...
                  await upsert("conversation_messages", {
                    ...
                    sender: "agent",
                    content: responseMessage.outputText,
  ...
  upsertZapi: async (req: Request, res: Response) => {
    try {
      ...
      let conversation;
      const integrations = await getByFilter("integrations", {
        "metadata->instance->>id": webhookContent.instanceId,
      });
      ...
      if (lead && lead.length > 0) {
        ...
        const newLead = await upsert("leads", {
          phone: webhookContent.phone,
          name: webhookContent.senderName || "Desconhecido",
      ...
      await upsert("conversation_messages", {
        conversation_id: conversation["id"],
        sender: "human",
        content: webhookContent.text.message,
        metadata: webhookContent,
        timestamp: new Date(webhookContent.momment),
      });

      if (conversation["mode"] === "agent") {
        const message = await AiExecutor.executeAgentText(
          agent["id"],
          conversation["lead_id"],
          webhookContent.text.message,
        );
  ...
  upsertUazapi: async (req: Request, res: Response) => {
    try {
      ...
      const integrations = await getByFilter("integrations", {
        "metadata->instance->>token": webhookContent.token,
      });
      ...
      await upsert("conversation_messages", {
        conversation_id: conversation["id"],
        sender: "human",
        content: webhookContent.message.text,
        metadata: webhookContent,
        timestamp: new Date(webhookContent.message.messageTimestamp),
      });
      ...
      if (conversation["mode"] === "agent") {
        ...
        if (webhookContent.message.type === "media") {
          const mediaContent = await getMediaContent(
            webhookContent.message.messageid,
            webhookContent.token,
          );
          message = await AiExecutor.executeAgentMedia(
            agent["id"],
            conversation["lead_id"],
            mediaContent,
            webhookContent.message.messageType === "ImageMessage" ? "image" : "audio",
          );
        } else {
          message = await AiExecutor.executeAgentText(
            agent["id"],
            conversation["lead_id"],
            webhookContent.message.text,
          );
        }
